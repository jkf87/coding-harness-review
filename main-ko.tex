...
그림~\ref{fig:overall-bar}은 6개 도구의 100점 만점 환산 점수를 보여준다.
상위 4개 도구와 하위 2개 도구의 격차는 주로 T5 가드레일 점수에서 발생한다.
...
\draw[->] (0,-0.2) -- (\maxw+1.2,-0.2) node[right] {\footnotesize 점수};
\foreach \x/\v in {0/0, 1.6/20, 3.2/40, 4.8/60, 6.4/80, 8.0/100} {
  \draw (\x,-0.2) -- (\x,-0.4) node[below,font=\scriptsize] {\v};
}
\end{tikzpicture}
\caption{100점 만점 환산 전체 점수. 상위 4개 도구(파란색/청록색)는 80점 이상, 하위 2개(주황색)는 75점 미만으로, 주로 T5 가드레일 점수에 의해 구분된다.}
...
\subsection{태스크별 점수 분포}

그림~\ref{fig:radar}는 도구별 태스크 점수 분포를 보여준다.
...
% Ouroboros
\foreach \v/\c/\x in {4.4/blue!30/0, 4.6/blue!45/0.24, 4.6/blue!60/0.48, 7.0/red!50/0.72} {
  \node[bar=\c,minimum height=\v*0.7cm] at (\x,0) {};
}
\node[font=\scriptsize,anchor=north] at (0.36,-0.1) {Ouro.};

% revfactory
\foreach \v/\c/\x in {4.0/teal!30/\gp, 4.2/teal!45/\gp+0.24, 4.5/teal!60/\gp+0.48, 6.0/red!50/\gp+0.72} {
  \node[bar=\c,minimum height=\v*0.7cm] at (\x,0) {};
}
...
\draw[->] (-0.2,0) -- (-0.2,6.0) node[above,font=\footnotesize] {점수};
...
\node[font=\scriptsize,anchor=west] at (1.2,5.8) {T2 계획};
\node[draw,fill=blue!45,minimum size=0.25cm] at (2.5,5.8) {};
\node[font=\scriptsize,anchor=west] at (2.7,5.8) {T3 구현};
\node[draw,fill=blue!60,minimum size=0.25cm] at (4.0,5.8) {};
\node[font=\scriptsize,anchor=west] at (4.2,5.8) {T4 리뷰};
\node[draw,fill=red!50,minimum size=0.25cm] at (5.5,5.8) {};
\node[font=\scriptsize,anchor=west] at (5.7,5.8) {T5 가드};
\end{tikzpicture}
\caption{도구별 태스크 점수. 파란색/청록색(T2--T4)은 도구 간 편차가 작고, 빨간색(T5 가드레일)이 가장 넓은 분포를 보인다.}
\label{fig:radar}
\end{figure}
...
세 가지 아키텍처 패턴을 식별했다 (그림~\ref{fig:arch}):
...
\end{tikzpicture}
\caption{평가된 하네스 도구에서 식별된 세 가지 아키텍처 패턴. (a)~명세 우선: 실행 전 제약을 불변 명세로 결정화. (b)~역할 기반: 전문화된 에이전트 역할 간 작업 분해. (c)~스킬팩: 기존 에이전트를 훅, 명령, 품질 게이트로 확장.}
\label{fig:arch}
\end{figure}
